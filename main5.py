...
"""
image_matrix: the image matrix,its type is narray
...
def bilateral_filter_image(image_matrix, window_length=7, sigma_color=20, sigma_space=20, mask_image_matrix=None):
    mask_image_matrix = np.zeros(
        (image_matrix.shape[0], image_matrix.shape[
            1])) if mask_image_matrix is None else mask_image_matrix  # default: filtering the entire image
    image_matrix = image_matrix.astype(
        np.int32)  # transfer the image_matrix to type int32，for uint cann't represent the negative number afterward
    ...
    look_for_gaussion_table_ufun = np.vectorize(look_for_gaussion_table, otypes=[
        np.float64])  # to accelerate the process of get the gaussion matrix about color.key:color difference，value:gaussion weight
    bilateral_filter_distance_matrix = generate_bilateral_filter_distance_matrix(window_length,
                                                                                 sigma_space)  # get the gaussion weight about distance directly
    ...
    for i in range(0 + margin, image_matrix.shape[0] - margin):
        for j in range(0 + margin, image_matrix.shape[1] - margin):
            if mask_image_matrix[i][j] == 0:
                ...
                bilateral_filter_matrix = bilateral_filter_matrix / np.sum(bilateral_filter_matrix, keepdims=False,
                                                                           axis=(0, 1))  # normalize the weigth matrix
                filter_output = np.sum(np.multiply(bilateral_filter_matrix, filter_input), axis=(
                0, 1))  # multiply the input window by the weigth matrix，then get the sum of channels seperately
                filter_image_matrix[i][j] = filter_output
    filter_image_matrix = limit_ufun(filter_image_matrix)  # limit the range
    filter_image_matrix=filter_image_matrix.astype(int)
...
image = cv2.imread('lionss.jpg', cv2.IMREAD_COLOR)
width, heigth = image.shape[:2]
final_wide = int(width / 2)
...
bilateral_filtered = bilateral_filter_image(image)
cv2.imwrite("filtered_image_Own.png", bilateral_filtered)

cv2.imwrite('diff.png', filtered_image_OpenCV-bilateral_filtered)

print("Open CV")
print(filtered_image_OpenCV[-2])

print("Filtered")
print(bilateral_filtered[-2])